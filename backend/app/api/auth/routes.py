...
def _user_public(user: User) -> UserPublic:
    return UserPublic(id=user.id, email=user.email, full_name=user.full_name, plan=user.plan.value)
...
def register(
    ...
    if users_repo.get_by_email(db, payload.email.lower()):
...
def me(user: User = Depends(get_current_user)) -> MeResponse:
    return MeResponse(id=user.id, name=user.full_name, email=user.email, plan=user.plan.value)
